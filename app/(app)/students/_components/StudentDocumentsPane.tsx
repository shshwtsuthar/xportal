...
} from '@/src/hooks/useStudentFiles';
...
export function StudentDocumentsPane({ studentId }: Props) {
  const {
    data: files = [],
    isLoading,
    isError,
  } = useListStudentFiles(studentId);
  const uploadMutation = useUploadStudentFile();
  const deleteMutation = useDeleteStudentFile();
  const signedUrlMutation = useCreateStudentSignedUrl();

  const onDrop = useCallback(
    async (acceptedFiles: File[]) => {
      if (!studentId) {
        toast.error('Student ID required to enable uploads.');
        return;
      }
  ...
  const isDisabled = useMemo(() => !studentId, [studentId]);
  ...
  const handleDelete = async (name: string) => {
    if (!studentId) return;
    try {
      await deleteMutation.mutateAsync({ studentId, fileName: name });
    ...
    <div className="space-y-4">
      ...
      >
        ...
        <p className="text-sm font-medium">
          {isDisabled
            ? 'Student ID required to enable uploads'
