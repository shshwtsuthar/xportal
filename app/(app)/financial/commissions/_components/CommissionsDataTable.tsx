...
import {
  useGetCommissionInvoices,
  type CommissionInvoiceFilters,
} from '@/src/hooks/useGetCommissionInvoices';
import { Input } from '@/components/ui/input';
import {
  getCommissionsTableKey,
...
import {
  ...
  type ColumnDef,
  type RowType,
} from './commissionsTableColumns';

export type CommissionsDataTableRef = { getRows: () => RowType[] };

type Props = { filters?: CommissionInvoiceFilters };

export const CommissionsDataTable = forwardRef<CommissionsDataTableRef, Props>(
  function CommissionsDataTable({ filters }: Props, ref) {
    const { data, isLoading, isError } = useGetCommissionInvoices(
      filters as CommissionInvoiceFilters | undefined
    );
    ...
    const tableKey = getCommissionsTableKey();
    ...
    const allColumns: ColumnDef[] = getCommissionsColumns();
    ...
    const filteredRows = useMemo(() => {
      ...
      return rows.filter((row) => {
        return visibleDefs.some((col) => {
          ...
          const isDateColumn = col.id === 'issue_date' || col.id === 'due_date';
    ...
    if (isError)
      return (
        <p className="text-destructive text-sm">Failed to load commissions.</p>
      );
        ...
        <div className="border-b p-3">
          <div className="relative">
            ...
            <Input
              ...
              aria-label="Search commissions table"
            />
          </div>
        </div>
        <Table>
