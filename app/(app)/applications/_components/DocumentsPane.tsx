...
export function DocumentsPane({ applicationId }: Props) {
  const {
    data: files = [],
    isLoading,
    isError,
  } = useListApplicationFiles(applicationId);
  const uploadMutation = useUploadApplicationFile();
  const deleteMutation = useDeleteApplicationFile();
  ...
  const onDrop = useCallback(
    async (acceptedFiles: File[]) => {
      ...
      for (const file of acceptedFiles) {
        ...
        try {
          await uploadMutation.mutateAsync({ applicationId, file });
  ...
  const isDisabled = useMemo(() => !applicationId, [applicationId]);
  ...
  const handleDelete = async (name: string) => {
    ...
    try {
      await deleteMutation.mutateAsync({ applicationId, fileName: name });
