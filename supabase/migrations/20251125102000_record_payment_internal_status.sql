...
-- Update record_payment to maintain internal_payment_status instead of mutating invoice.status.
...
BEGIN
  ...
  SET amount_paid_cents = amount_paid_cents + p_amount_cents,
      internal_payment_status = CASE
        WHEN amount_paid_cents + p_amount_cents <= 0 THEN 'UNPAID'::public.internal_payment_status
        WHEN amount_paid_cents + p_amount_cents < amount_due_cents THEN 'PARTIALLY_PAID'::public.internal_payment_status
...
COMMENT ON FUNCTION public.record_payment IS 'Records a payment against an invoice, updates internal_payment_status, and returns the payment_id for commission calculation.';
