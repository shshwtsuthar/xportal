...
-- Provide INV-YYYY-XXXXX generator for invoice_number
...
BEGIN
  ...
  LOOP
    SELECT EXISTS (
      SELECT 1 FROM public.invoices WHERE invoice_number = v_final_id
    )
    INTO v_exists;
...
COMMENT ON FUNCTION public.generate_invoice_number(date, uuid) IS
  'Generates INV-YYYY-XXXXX invoice numbers using a deterministic hash of the invoice UUID.';
