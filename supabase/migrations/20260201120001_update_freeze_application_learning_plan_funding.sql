-- Add AVETMISS NAT00120: freeze_application_learning_plan with funding_source_national (part 2/3)
...
BEGIN
  ...
  -- Prerequisite insert (with funding_source_national)
  WITH plans AS (
    SELECT pp.*
  ...
  SELECT app_id,
         ...
         true,
         pr.sequence_order,
         pr.funding_source_national
  FROM prerequisite_records pr;

  -- Current cycle subjects (with funding_source_national)
  WITH plans AS (
    SELECT pp.*
    FROM public.timetable_program_plans tpp
    JOIN public.program_plans pp ON pp.id = tpp.program_plan_id
  ...
  SELECT app_id, c.id, c.subject_id, c.start_date, c.end_date, false, false, c.sequence_order,
         c.funding_source_national
  FROM cur c;

  -- Catch-up subjects (with funding_source_national)
  WITH plans AS (
    SELECT pp.*
    FROM public.timetable_program_plans tpp
    JOIN public.program_plans pp ON pp.id = tpp.program_plan_id
