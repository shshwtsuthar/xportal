...
/**
 * Internal helper to calculate the expected check digit for USI validation.
 */
const generateUSICheckCharacter = (input: string): string => {
  let factor = 2;
  let sum = 0;
  const n = USI_VALID_CHARS.length;

  // Work from right to left
  for (let i = input.length - 1; i >= 0; i--) {
    const codePoint = USI_VALID_CHARS.indexOf(input[i]);
  ...
  return USI_VALID_CHARS[checkCodePoint];
};
 ...
 */
const verifyUSI = (usi: string): boolean => {
  ...
  try {
    ...
    const checkDigit = generateUSICheckCharacter(inputUpper.substring(0, 9));
